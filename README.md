...
### Infrastructure — Kubernetes (alternative)
...
Déploiement alternatif sur un cluster Kubernetes local (minikube). Les manifests sont dans `k8s/`.
...
Le Service est de type `ClusterIP` (interne au cluster). Utiliser `port-forward` pour y accéder depuis le Mac :
...
kubectl logs -n airflow deployment/airflow-dag-processor

# Décrire un pod (events, probe failures, etc.)
kubectl describe pod -n airflow <nom-du-pod>

# Exécuter un shell dans un container
kubectl exec -it -n airflow deployment/airflow-scheduler -- bash

# Arrêter/relancer minikube (les pods et données sont conservés)
minikube stop
minikube start
```

### Différences Docker Compose → Kubernetes

| Docker Compose | Kubernetes | Notes |
|---|---|---|
| `environment:` / `env_file:` | `ConfigMap` + `Secret` | Séparation sensible / non-sensible |
| `volumes: ./dags:/opt/airflow/dags` | `COPY` dans le Dockerfile | Tout est dans l'image (immuable) |
| `volumes: postgres-db-volume:` | `PersistentVolumeClaim` | Disque persistant alloué par K8s |
| `ports: "8080:8080"` | `Service` + `port-forward` | ClusterIP en dev, Ingress en prod |
| `depends_on:` | `initContainers` | Vérification active avant démarrage |
| `restart: always` | `restartPolicy: Always` (défaut) | K8s redémarre automatiquement |
| Service `airflow-init` | `Job` | Pod à usage unique, s'arrête après complétion |
